...
class Controller_Component_Notify extends \sowerphp\core\Controller_Component
{
    ...
    private function sendEmail($from, $to, $message)
    {
        ...
        if ($from) {
            ...
            $aux = config('mail.from');
            $email->replyTo($aux['address'], $aux['name']);
        }
        $To = is_object($to) ? $to : new $this->settings['model']($to);
        $email->to($To->email);
        $timestamp = microtime(true);
        // asunto
        $email->subject('['.config('app.name').'] Notify '.$this->getFacility($from).'.'.$this->getSeverity($message['gravedad']).' '.$timestamp);
        // mensaje
