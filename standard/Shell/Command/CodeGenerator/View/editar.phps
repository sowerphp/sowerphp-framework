...
foreach($columns as $column => &$name) {
	...
	if(is_array($name) || !$columnsInfo[$column]['auto']) {
		...
		// si es de tipo text se muestra un textarea
			...
			echo $form->input(array('type'=>'textarea', 'name'=>$column, 'label'=> $name, 'help' => $columnsInfo[$column]['comment'], 'check' => (!$columnsInfo[$column]['null']?'notempty':''), 'value'=>${class}->{$column}));
		}
		// si es de tipo boolean se muestra lista desplegable
		else if($columnsInfo[$column]['type']=='boolean') {
			echo $form->input(array('type'=>'select', 'name'=>$column, 'label'=> $name, 'options' => $optionsBoolean, 'help' => $columnsInfo[$column]['comment'], 'check' => (!$columnsInfo[$column]['null']?'notempty':''), 'selected' => ${class}->{$column}));
		}
		// si es de tipo date se muestra calendario
		else if($columnsInfo[$column]['type']=='date') {
			echo $form->input(array('type'=>'date', 'name'=>$column, 'label'=> $name, 'help' => $columnsInfo[$column]['comment'], 'check' => (!$columnsInfo[$column]['null']?'notempty':''), 'value' => ${class}->{$column}));
		}
		// si es llave foránea
			...
			echo $form->input(array('type'=>'select', 'name'=>$column, 'label'=> $name, 'options' => $options, 'help' => $columnsInfo[$column]['comment'], 'check' => (!$columnsInfo[$column]['null']?'notempty':''), 'selected' => ${class}->{$column}));
		}
		...
		// si es cualquier otro tipo de datos
			...
			echo $form->input(array('name'=>$column, 'label'=>$name, 'help' => $columnsInfo[$column]['comment'], 'check' => (!$columnsInfo[$column]['null']?'notempty':''), 'value' => ${class}->{$column}));
		}
	}
}
